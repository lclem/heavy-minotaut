...
void simple_Heavy_tester(unsigned int la_dw, unsigned int la_up,
                         string input_dir, string output_dir)
{
    ...
    for (unsigned int i=0; i<NUMB_TESTS; i++)
    {
        ...
        stateDict sDict;
        AutData autData_i = parseFromString(autStr, sDict, false, false);
        ...
        try
        {
            AutData autData_heavy = heavy(autData_i, la_dw, la_up);

            aut_heavy  =  getAut(autData_heavy);
                  ...
                  vector<string> dir, string summarized_results_filename = "")
{
    ...
    for (unsigned int i=0; i<NUMB_TESTS  ; i++)
    {
        ...
        /* Parsing and taking data from the automaton. */
        stateDict stateDict;
        AutData autData = parseFromString(autStr,stateDict,false,false);

        Automaton aut = getAut(autData);
        getNumbUsedStates(aut);
        ...
        applyHeavy(autData, stateDict, metaData, la_dw, la_up,
                   testData_heavy, timeout_heavy,
                   /*log_humanread_filename,*/
                   log_machread_filename, log_machread_heavy_times_filename);
...
void call_heavy_tester(unsigned int la_dw, unsigned int la_up, string dir)
{
    ...
    for (const string& subfolder : subfolders)
    {
        ...
        heavy_tester(la_dw, la_up, {subdir}, summarized_results_filename);

    }

}

void call_combined_relation_tester(unsigned int la_dw, unsigned int la_up, string dir)
{

    vector<string> subfolders = getAllSubfolders(dir);

    string summarized_results_filename =
            "very_summarized_results_combined_relation_la-dw=" + std::to_string(la_dw) + "_la-up="
            + std::to_string(la_up) + "_" + localTime() + ".txt";
    ...
    for (const string& subfolder : subfolders)
    {
        ...
        combined_relation_tester(la_dw, la_up, {subdir}, summarized_results_filename, log_humanread_filename);
    }
...
void compare_pr_methods(string dir)
{
    ...
    for (const string& subfolder : subfolders)
    {
        ...
        for (unsigned int i=0; i<NUMB_TESTS ; i++)
        {
            ...
            outputText(std::to_string(i+1) + ": " + filenames.at(i) + "\t",
            ...
            vector<vector<bool> > W1;
            W1 = dw_simulation(autData, pr_depth);
            ...
            for (unsigned int i=0; i<W1.size() && i != greatest_state; i++)
                for (unsigned int j=0; j<W1.size() && j != greatest_state; j++)
                    if (W1.at(i).at(j) && !W2.at(i).at(j))
                        outputText("(" + translateState(aut,stateDict,i) + ","
                                   + translateState(aut,stateDict,j) + ") ");
...
void minimizationTechniques_tester()
{
    ...
    call_heavy_tester(2, 4, "test automata//moderate");
