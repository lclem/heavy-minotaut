                                  ...
                                  string log_humanread_filename)
{
    ...
    extractStrictRelation(up_la_with_id_1st_time);
    start = std::chrono::high_resolution_clock::now();
              ...
              TestData& testData, string log_humanread_filename)
{
    ...
    seconds timeout_start = secsSinceEpoch();
...
AutData heavyGradual(const AutData& autData_i,
                     ...
                     unsigned int greatest_symbol,
                     TestData& testData, seconds timeout_start, string log_humanread_filename)
{
                    ...
                    string log_machread_heavy_times_filename)
{
    ...
    metaData.inc();
    metaData.updateInitialAvg(getNumbUsedStates(aut_i,true), getNumbTransitions(aut_i,true),
                               getNumbSymbols(aut_i,true), getAvgRank(aut_i),
                               getTransitionDensity(aut_i,true),
                               measureTransOverlaps(aut_i));
    metaData.checkInitialGreatest(getNumbUsedStates(aut_i,true),
                                   getNumbTransitions(aut_i,true),
                                   getNumbSymbols(aut_i,true), getAvgRank(aut_i),
                                   getTransitionDensity(aut_i,true),
                                   measureTransOverlaps(aut_i));
    ...
    if (log_machread_heavy_times_filename != "")
        writeToFile(log_machread_heavy_times_filename,
                    std::to_string(getNumbUsedStates(aut_i,true)) + "\t"
                    + std::to_string(getNumbTransitions(aut_i,true)) + "\t");
    ...
    {
        auto start = std::chrono::high_resolution_clock::now();
        AutData autData_quot = quotient_with_dw_la_sim(autData_i, 1,
    ...
    catch (timeout_& e)
    {
        outputText("TIMEOUT: The Quotienting method took longer than "
                  + std::to_string( (float) TIMEOUT/60 ) + " minutes and therefore was aborted.\n",
                   log_humanread_filename);
        timeout_quot.inc();
        timeout_quot.checkSmallest(getNumbUsedStates(aut_i,true),
                                    getNumbTransitions(aut_i,true),
                                    getTransitionDensity(aut_i,true));
    ...
    catch (timeout_& e)
    {
        ...
        timeout_qAndP.inc();
        timeout_qAndP.checkSmallest(getNumbUsedStates(aut_i,true),
                                    getNumbTransitions(aut_i,true),
                                    getTransitionDensity(aut_i,true));
    ...
    catch (timeout_& e)
    {
        outputText("TIMEOUT: The Heavy(3,7) took longer than "
                  + std::to_string( (float) TIMEOUT/60 ) + " minutes and therefore was aborted.\n",
                   log_humanread_filename);
        timeout_heavy37.inc();
        timeout_heavy37.checkSmallest(getNumbUsedStates(aut_i,true), getNumbTransitions(aut_i,true), getTransitionDensity(aut_i,true));
                ...
                string log)
{
    ...
    try
    {
        autData_heavy = heavy(autData_i, la_dw, la_up, testData_heavy);
    }
    catch (timeout_& e)
    {
        timeout_heavy.inc();
        timeout_heavy.checkSmallest(getNumbUsedStates(aut_i,true), getNumbTransitions(aut_i,true), getTransitionDensity(aut_i,true));
                               ...
                               string log_machread_times_filename)
{
    ...
    metaData.inc();
    metaData.updateInitialAvg(getNumbUsedStates(aut,true), getNumbTransitions(aut,true),
                               getNumbSymbols(aut,true),   getAvgRank(aut),
                               getTransitionDensity(aut,true),
                               measureTransOverlaps(aut));
    metaData.checkInitialGreatest(getNumbUsedStates(aut,true), getNumbTransitions(aut,true),
                                  getNumbSymbols(aut,true),    getAvgRank(aut),
                                  getTransitionDensity(aut,true),
                                  measureTransOverlaps(aut));
    ...
    catch (timeout_& e)
    {
        outputText("TIMEOUT: The Quotienting method took longer than "
                  + std::to_string( (float) TIMEOUT/60 )
                   ...
                   log_humanread_filename);
        timeout.inc();
        timeout.checkSmallest(getNumbUsedStates(aut,true), getNumbTransitions(aut,true), getTransitionDensity(aut,true));
