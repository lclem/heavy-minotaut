...
float refine(const bool* isFinal, const vector<typerank> ranks,
...
vector<vector<vector<unsigned int> > > getPostLen(const vector<vector<pair<transition,size_t>> >& trans_botup,
    ...
    for (unsigned int state=0; state<size; state++) {
        ...
        // trans that have 'state' has one of the children
...
vector<vector<bool> >& pre_refine(vector<vector<bool> >& W, const vector<vector<vector<unsigned int> > >& post_len,
                                  const unsigned int numb_states, const unsigned int numb_symbols) {
...
float up_simulation(const Automaton& aut, const unsigned int la,
    ...
    float refin = refine(isFinal, ranks, trans_botup, la, param_rel, param_strict, W, n);
